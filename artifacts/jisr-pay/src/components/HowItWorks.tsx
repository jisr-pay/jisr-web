  ...
  return (
    <section className="w-full max-w-6xl mx-auto px-4 py-20 md:py-28">
        ...
        {steps.map((s, i) => {
          ...
          return (
            <motion.div
              ...
              className={`group relative rounded-2xl border border-border bg-card p-7 backdrop-blur-sm transition-colors ${s.ring} overflow-hidden`}
            >
              {/* metallic sheen */}
              <div className="pointer-events-none absolute -top-24 -end-24 h-48 w-48 rounded-full bg-primary/5 blur-2xl" />
              <div className="flex items-center justify-between mb-5">
                <span className={`inline-flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 border border-primary/20 ${s.accent}`}>
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-6 rounded-2xl border border-primary/20 bg-primary/5 p-8"
      >
        {stats.map((st) => (
