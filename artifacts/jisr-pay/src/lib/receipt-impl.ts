/**
 * Receipt PDF implementation — split from receipt.ts so jsPDF (~350 KB)
 * lands in its own lazy chunk and only downloads when a user actually
 * requests a receipt.
 *
 ...
 *  - Details grid (hash, recipient, fee, time)
...
export function generateReceiptPDFImpl(data: ReceiptData): void {
  ...
  doc.setFontSize(9);
  rgb(doc, GOLD);
  const savingsStr = 'Stellar Testnet payment - no local-currency conversion';
